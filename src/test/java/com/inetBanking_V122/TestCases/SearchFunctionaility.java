...
public class SearchFunctionaility extends BaseClass {
	...
	@Test(priority = 11)
	public void ValidatethatwhenAddtocartlabelchangestoAddedwhenuserclickontheaddtocartbutton()
			throws InterruptedException {
		sp = new SearchPage(driver);
		s = new Sleep(driver);
		s.waitForSomeSecondsHardSleep(2000);
	...
	@Test(priority = 12)
	public void ValidatethatAddedlabelchangestoAddtocartaftersometimeofaddingtheproducttocart()
			throws InterruptedException {
		sp = new SearchPage(driver);
		s = new Sleep(driver);
		s.waitForSomeSecondsHardSleep(2000);
		Assert.assertEquals(sp.getTextfteraddingCartToProduct("Tomato"), "ADD TO CART");

	}

	@Test(priority = 13)
	public void Validatethatwhenproductareaddedtothecartpricefieldisupdated() throws InterruptedException {
		sp = new SearchPage(driver);
		s = new Sleep(driver);
		s.refreshPage();
		s.waitForSomeSecondsHardSleep(2000);
